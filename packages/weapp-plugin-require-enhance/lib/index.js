...
var npmHack = function npmHack(libResolvedPath, contents) {
  ...
  // 这里在编译这一步直接做替换 否则报错
  contents = contents.replace(/process\.env\.NODE_ENV/g, (0, _stringify2.default)(process.env.NODE_ENV));
  ...
  if (libResolvedPath.match('core-js/library/modules/_microtask.js')) {
    ...
    contents = contents.replace('Promise && Promise.resolve', 'false && Promise && Promise.resolve');
  }

  if (libResolvedPath.match('core-js/library/modules/_freeGlobal.js') || libResolvedPath.match('lodash/_freeGlobal.js')) {
    contents = contents.replace('module.exports = freeGlobal;', 'module.exports = freeGlobal || this;');
  }
...
var checkDeps = function checkDeps(dependCode, dependResolvedPath, dependDistPath, npmInfo, config, log, extra, plgConfig) {
  var distCode = dependCode.replace(/require\(['"]([\w\d_\-./@]+)['"]\)/gi, function (match, lib) {
    ...
    if (lib.indexOf('/') === -1 // require('asset');
    ) {
        try {
          libResolvedPath = resolveCwd(lib);
          var libBaseName = basename(libResolvedPath);
          libResolvedDistPath = join(config.resolvedDist, npmFolder, libBaseName === 'index.js' ? utils.addExt(lib) : libBaseName);
        } catch (error) {
          ...
          if (error.code === 'MODULE_NOT_FOUND') {
            if (!isNpm) {
              ...
              log.warn('[weapp-plugin-require]: 建议小程序中 require module 的路径，使用显示的相对路径 "./module" 来代替 "module" :' + match);
    ...
    if (lib.indexOf('/') !== -1 && // require('lodash/_arrayEach')
    !lib.startsWith('/') && !lib.startsWith('.')) {
      ...
      try {
        ...
        if (error.code === 'MODULE_NOT_FOUND') {
          ...
          try {
            for (var _iterator = (0, _getIterator3.default)(resolveModules), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      if (utils.isDir(join(libResolvedPathOpts.dir, inner))) {
        ...
        libResolvedDistPath = join(config.resolvedDist, npmFolder, mainNpm, utils.addExt(inner, '.js'));
      }
      ...
      _contents = checkDeps(_contents, libResolvedPath, libResolvedDistPath, {}, config, log, extra);

      extra[libResolvedDistPath] = { contents: _contents };
    }

    if (lib.startsWith('.') // require('./ohter');
    ) {
        if (isNpm) {
          ...
          _contents2 = checkDeps(_contents2, libResolvedPath, libResolvedDistPath, {}, config, log, extra);
    ...
    if (isAbsolute(lib) // require('/com/button');
    ) {
        // 对配置的路径绝对路径进行处理
...
exports.default = (0, _weappUtilCreatePlugin2.default)({
  ...
  file.contents = checkDeps(file.contents.toString(), dependResolvedPath, dependDistPath, false, utils.config, utils.log, file.extra, plgConfig);
